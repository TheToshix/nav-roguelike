...
void Game::quaff(const Item& it) {
    ...
    switch (static_cast<PotionKind>(it.subtype)) {
        case PotionKind::Heal: {
            ...
            hero_.a.heal(amount);
            // Снимает и яд — потому что иначе ответа на яд у игрока почти нет.
            // Яд отнимает 2 здоровья за ход, а отдых возвращает 1 за двенадцать:
            // отравленный теряет в двадцать четыре раза быстрее, чем чинится, и
            // пересидеть это нельзя. При этом каждое новое попадание обновляет
            // срок, так что стрелок вроде Аспида держит яд постоянно. Раньше
            // единственным лекарством было великое исцеление — замер показал
            // 1,2 штуки на пять этажей. Ответ на частую угрозу не может быть
            // редчайшим предметом в игре.
            //
            // Великое исцеление от этого не обесценивается: оно лечит заметно
            // больше и снимает вдобавок огонь.
            hero_.a.clear_effect(Effect::Poison);
