...
const std::vector<Species>& bestiary() {
    static const std::vector<Species> table = {
        ...
        // Волколак и Лихо начинаются со второго пояса, а не с третьего этажа.
        // Замер на 80 партиях честного бота: раньше 59 из них кончались на
        // третьем-четвёртом этаже, потому что там сходились сразу трое —
        // меньший страж, хозяин пояса и новый ярус обычной нечисти. Вий должен
        // быть пиком, но приходил не один: на него приходилось 24 смерти из 80.
        // После сдвига — 45 партий вместо 59, до второго пояса доходит 35
        // вместо 21, на Вия остаётся 12.
        //
        // Окна сужены сверху ровно настолько, чтобы вес на седьмом этаже и
        // глубже остался прежним. Первая попытка сдвигала окна целиком, и это
        // уплотняло Волколака на восьмом этаже (12 → 16): обход подземелья
        // тут же завис у Бабы-Яги на шестидесяти тысячах ходов.
        {"volkolak",  Text{"Волколак", "Volkolak"},               'V', "#9b7b4b", 22,  9,  2, 130, 9,  20,  5,  9,  8,
         AiMelee, Effect::Poison, 0, 0,
        ...
        {"likho",     Text{"Лихо Одноглазое", "Likho"},           'L', "#8b5a8b", 30, 12,  3, 100, 8,  30,  6, 10,  7,
