...
`Shift` и направление бежит по коридору до развилки, `o` доводит до ближайшего места, где
герой ещё не был, `m` показывает весь изученный этаж целиком с лестницей вниз. Шестнадцать
этажей, пройденных по одной клетке за нажатие, — это главное, что утомляло в игре, и никаких
решений в пустом коридоре не принимается.

Оба перемещения — это обычные шаги, которые движок делает сам, а не новый способ ходить:
голод, яд, ходы монстров и все инварианты работают в беге ровно так же, как в шаге. Интересна
в них не езда, а тормоза. Бег останавливается, когда кто-то появился в виду, когда героя
ранили, когда подействовало что-то новое, когда под ногами вещь, лестница или капище и когда
коридор предлагает выбор. Автообход вообще отказывается идти, если рядом кто-то есть.
...
```bash
ctest --test-dir build --output-on-failure    # 365 тестов
...
| | Классическая | WASD |
|---|---|---|
| Идти | `hjkl` `yubn` | `wasd` `qezc` |
| Бежать до развилки | `Shift` + то же | `Shift` + то же |
| Бросить вещь | `d` | `r` |
| Колдовать | `z` | `f` |
| Выйти | `q` | через меню |
...
[багрепортах](docs/BUG_REPORTS.md), где записаны четырнадцать дефектов, найденных в ходе разработки,
вместе с шагами воспроизведения и правками.

| | |
|---|---|
| Юнит- и интеграционных тестов | **365** в 49 наборах |
