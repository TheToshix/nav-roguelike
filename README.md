...
Проверяется это не на слух. `tests/web/music.test.mjs` поднимает headless-браузер, рендерит
каждую из восьми тем в `OfflineAudioContext` и меряет: сигнал есть у всех трёх фаз, громкость
третьей фазы минимум в 1,3 раза выше первой, микс не клиппует, а форма волны на смене фазы
остаётся непрерывной.

Последняя проверка — страховка, а не измерение, и честнее сказать почему. Смена фазы здесь
делается подъёмом громкости на непрерывных осцилляторах, поэтому большой разрыв она дать не
может в принципе: при штатном кроссфейде в 1,5 с самый крупный скачок по всем темам — 6,9% от
пика, а если кроссфейд урезать до полумиллисекунды, он вырастает всего до 10,0%. Тест этого не
поймает, и никакой порог, который поймал бы, не отстоял бы от 6,9% достаточно, чтобы не
мигать. Ловит он другое — переход, переписанный на подмену буфера или перезапуск осциллятора,
то есть настоящий обрыв.
...
```bash
ctest --test-dir build --output-on-failure    # 459 тестов
...
| | |
|---|---|
| Юнит- и интеграционных тестов | **459** в 59 наборах |
...
| Дополнительно в CI | ASan + UBSan, `-Werror`, отчёт покрытия, 20 полных партий без экрана, обходчик до 16 этажа, сборка WebAssembly, ESLint по коду страницы, замер восьми тем стражей в headless-браузере |
...
tests/           459 тестов на GoogleTest
